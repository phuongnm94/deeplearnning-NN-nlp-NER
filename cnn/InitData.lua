...
--inputParser = DataInputsCNNParser:new(nil)
--print(inputParser:show())
...
function loadDataSet(sPathFileDataSet, pairWordIds, NERIds,nLastIdxMtWordVector, sizeAppendDict)
        ...
        -- load data file
        local inputs, targets, sizeSentencesInfo, features
        if(g_isUseFeatureWord == true) then
                inputs, targets, sizeSentencesInfo, features=
                        getDataSentenceFrom2(sPathFileDataSet,pairWordIds,NERIds,nLastIdxMtWordVector, sizeAppendDict)
        else
                inputs, targets, sizeSentencesInfo, features =
                        getDataSentenceFrom(sPathFileDataSet,pairWordIds,NERIds,nLastIdxMtWordVector, sizeAppendDict)
        end

        g_sizeSentencesInfo = sizeSentencesInfo
...
function InitData(sDictName, sDataSetName)
        ...
        if(bTestDebug == true) then
                ...
                -- appendPadding
                local nCountPadding = g_wordPaddingInfo["size"]
                if(nCountPadding ~=nil and nCountPadding > 0) then
                        for i=1, nCountPadding do
                                g_wordPaddingInfo["lstid"][i] = nLastIdxMtWordVector+i;
                        end
                end
                nLastIdxMtWordVector = nLastIdxMtWordVector + nCountPadding
                
                assert(sizeAppendDict > 0 ,"Please Increase value of rawDataInputSize")
                

                -- Luu cac vector cua cac tu ra bien toan cuc
